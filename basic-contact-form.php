...
function bcf_render_contact_form() {
    ...
    if (isset($_POST['submitted'])) {
        ...
        $html .= '<div class="bcf-error">You failed to prove you are human.</div>';
    ...
    $html .= '</div><div id="contact-respond"><form action="'. get_permalink(). '" method="post">';
    ...
    if ($options['bcf_spamtrap'] == 1) $html .= '<label for="message_human">Prove You\'re Human: <span>*</span> <br><div class="bcf-question">'.$options['bcf_question'].'</div> <input type="text" id="human-verify-input" name="message_human"></label>';
    $html .= '<input type="hidden" name="submitted" value="1"><input class="bcf-submit" type="submit"></p></form></div>';
    return $html;
}